...
int update_array_check(char *rules, int pmax, int *pa, int len, int *i_p, int *j_p);
void update_array_swap(int *pa, int len, int i, int j);
...
int main(int argc, char *argv[]){
	...
	for(update=update_list;update!=NULL;update=update->next){
		check = update_array_check(rules, pmax, update->pa, update->len, &i, &j);
		if(check == 0){
			limit = 0;
			do{
				update_array_swap(update->pa, update->len, i, j);
				check = update_array_check(rules, pmax, update->pa, update->len, &i, &j);
				limit++;
			}while(check == 0 && limit < 1000);
			assert(check);
